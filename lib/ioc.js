...
IOCContainer.prototype._initBeans = function(cb){
	...
	var tasks = {};
	Object.keys(self._initTasks).forEach(function(key) {
		var task = self._initTasks[key];
		tasks[key] = Array.isArray(task) && task.length == 1 ? task[0] : task;
	});
    async.auto(tasks, cb);
};
...
IOCContainer.prototype._autowirePropertiesByName = function(){
    ...
    for (var beanName in self.beans){
		...
		Object.keys(bean).forEach(function(k) {
			// for (var k in bean){
			if (bean[k] !== undefined){
				// continue;
			}
